...
def create_beatmap_object_from_api(raw_api_result: dict) -> Beatmap:
    return Beatmap(
        ...
        count_total=int(raw_api_result['count_normal']) + int(raw_api_result['count_slider']) + int(
            raw_api_result['count_spinner']),
...
def create_beatmapset_object_from_api(raw_api_result: dict) -> BeatmapSet | None:
    ...
    return BeatmapSet(
        ...
        bpm=float(raw_api_result['bpm']),
        approved=int(raw_api_result['approved']),
        approved_date=datetime.strptime(raw_api_result['approved_date'], '%Y-%m-%d %H:%M:%S') if raw_api_result['approved_date'] else None,
        submit_date=datetime.strptime(raw_api_result['submit_date'], '%Y-%m-%d %H:%M:%S') if raw_api_result['submit_date'] else None,
        last_update=datetime.strptime(raw_api_result['last_update'], '%Y-%m-%d %H:%M:%S') if raw_api_result['last_update'] else None,
